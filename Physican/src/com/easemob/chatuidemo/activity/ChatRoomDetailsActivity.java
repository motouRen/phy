...
public class ChatRoomDetailsActivity extends BaseActivity implements
	...
	protected void updateRoom() {
		new Thread(new Runnable() {
			public void run() {
				try {
					final EMChatRoom returnRoom = EMChatManager.getInstance()
							.fetchChatRoomFromServer(roomId);

					runOnUiThread(new Runnable() {
						public void run() {
							((TextView) findViewById(R.id.group_name))
									.setText(returnRoom.getName() + "("
											+ returnRoom.getAffiliationsCount()
											+ "人)");
	...
	public void onClick(View v) {
		...
		case R.id.clear_all_history: // 清空聊天记录
			...
			Intent intent = new Intent(ChatRoomDetailsActivity.this,
					AlertDialog.class);
