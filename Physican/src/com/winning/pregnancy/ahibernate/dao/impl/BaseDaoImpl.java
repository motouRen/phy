package com.winning.pregnancy.ahibernate.dao.impl;
...
import com.winning.pregnancy.ahibernate.annotation.Sole;
import com.winning.pregnancy.ahibernate.annotation.Table;
import com.winning.pregnancy.ahibernate.dao.BaseDao;
import com.winning.pregnancy.ahibernate.util.TableHelper;
 ...
 * 欢迎访问我的博客:http://blog.csdn.net/lk_blog, 这里有这个框架的使用范例和源码,
 * 希望朋友们多多交流完善这个框架,共同推动中国开源事业的发展,AHibernate期待与您共创美好未来!!!
 */
public class BaseDaoImpl<T> implements BaseDao<T>
{
	...
	private String soleColumn;
	private Class<T> clazz;
	private List<Field> allFields;
	private Field sColumn;
	...
	public BaseDaoImpl(SQLiteOpenHelper dbHelper, Class<T> clazz)
	{
		...
		for (Field field : this.allFields)
		{
			if (field.isAnnotationPresent(Sole.class))
			{
				Column column = (Column) field.getAnnotation(Column.class);
				this.soleColumn = column.name();
				this.sColumn = field;
				break;
			}
		}
	...
	public synchronized T find(String soleid)
	{
		String selection = this.soleColumn + " = ?";
		String[] selectionArgs = { soleid };
		Log.d(TAG, "[get]: select * from " + this.tableName + " where " + this.soleColumn + " = '" + soleid + "'");
		List<T> list = find(null, selection, selectionArgs, null, null, null, null);
		if ((list != null) && (list.size() > 0))
		{
			return (T) list.get(0);
	...
	public synchronized List<T> find(String[] columns, String selection, String[] selectionArgs, String groupBy, String having, String orderBy,
			String limit)
	{
		Log.d(TAG, "[find]");
	...
	private synchronized void getListFromCursor(List<T> list, Cursor cursor) throws IllegalAccessException, InstantiationException
	{
	...
	public synchronized long insertMore(List<T> entityList)
	{
		return insertMore(entityList, true);
	}

	public synchronized long insertMore(List<T> entityList, boolean flag)
	{
		String sql = "";
		SQLiteDatabase db = null;
		try
		{
			...
			for (int i = 0; i < entityList.size(); i++)
			{
				...
				if (flag)
				{
					sql = setContentValues(entityList.get(i), cv, TYPE_INCREMENT, METHOD_INSERT);// id自增
				}
				else
				{
					sql = setContentValues(entityList.get(i), cv, TYPE_NOT_INCREMENT, METHOD_INSERT);// id需指定
				}
	...
	public synchronized long insert(T entity, boolean flag)
	{
		String sql = "";
		SQLiteDatabase db = null;
		try
	...
	public synchronized void updateByColumn(T entity)
	{
		SQLiteDatabase db = null;
		try
		{
			...
			String column = cv.get(this.soleColumn).toString();
			String where = this.soleColumn + " = '" + column + "'";
			cv.remove(this.soleColumn);
	...
	public synchronized void updateMoreByColumn(List<T> entityList)
	{
		SQLiteDatabase db = null;
		try
		{
			...
			for (int i = 0; i < entityList.size(); i++)
			{
				...
				String sql = setContentValues(entityList.get(i), cv, TYPE_NOT_INCREMENT, METHOD_UPDATE);
				String column = cv.get(this.soleColumn).toString();
				String where = this.soleColumn + " = '" + column + "'";
				cv.remove(this.soleColumn);
				cv.remove(this.idColumn);
	...
	public synchronized void saveOrUpdate(T entity)
	{
		...
		try
		{
			db = this.dbHelper.getWritableDatabase();
			ContentValues cv = new ContentValues();
			this.sColumn.setAccessible(true);
			value = this.sColumn.get(entity) == null ? "" : this.sColumn.get(entity).toString();
			String sql = "select 1 from " + this.tableName + " where 1=1 and " + this.soleColumn + " = '" + value + "'";
			...
			{
				setContentValues(entity, cv, TYPE_INCREMENT, METHOD_INSERT);// id自增
	...
	public synchronized void saveOrUpdateMore(List<T> entityList)
	{
		SQLiteDatabase db = null;
		Cursor cursor = null;
		String value = "";
		try
		{
			...
			{
				this.sColumn.setAccessible(true);
				value = this.sColumn.get(entityList.get(i)) == null ? "" : this.sColumn.get(entityList.get(i)).toString();
				ContentValues cv = new ContentValues();
				String sql = "select 1 from " + this.tableName + " where 1=1 and " + this.soleColumn + " = '" + value + "'";
				cursor = db.rawQuery(sql, new String[] {});
				if (cursor.getCount() > 0)
				{
					setContentValues(entityList.get(i), cv, TYPE_NOT_INCREMENT, METHOD_UPDATE);
					String column = cv.get(this.soleColumn).toString();
					String where = this.soleColumn + " = '" + column + "'";
				...
				{
					setContentValues(entityList.get(i), cv, TYPE_INCREMENT, METHOD_INSERT);// id自增
					db.insert(this.tableName, null, cv);
