 ...
 * @version
 */
public class InquiryStateHandler
{
    private Context context;
    private static final int WHAT_DID_LOGIN_SUCC = 1;
    private static final int WHAT_DID_LOGIN_FAIL = 2;

    public InquiryStateHandler(Context context)
    {
        super();
        this.context = context;
    }

    public void handingBusiness(final HandingBusinessInf businessInf, final String inquiryID)
    {
        ...
        Thread thread = new Thread()
        {
            ...
            public void run()
            {
                ...
                params.add(new BasicNameValuePair("inquiryID", inquiryID));
                JSONObject json = HTTPGetTool.getTool().post(URLUtils.HOSTMAIN + URLUtils.URLINQUIRYBYKEY, params);
                try
                {
                    if (json != null)
                    {
